...
if ( ! function_exists( 'write_blog_posted_on' ) ) :
	...
	function write_blog_posted_on() {
		...
		if(is_single()){
            ...
            $posted_on = sprintf(
            /* translators: %s: post date. */
                esc_html_x( ' %s', 'post date', 'write-blog' ),
                '<a href="' . esc_url( get_day_link( $archive_year, $archive_month, $archive_day) ) . '" rel="bookmark">' . $time_string . '</a>'
            );
        }
		...
		if(is_single()){
            ...
            if ( $categories_list ) {
                ...
                printf( '<span class="cat-links"><span class="thememattic-icon ion-ios-folder-outline"></span>' . esc_html__( ' %1$s', 'write-blog' ) . '</span>', $categories_list ); // WPCS: XSS OK.
            }
        }
	}
endif;
	...
	function write_blog_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links"><span class="thememattic-icon ion-ios-pricetags-outline"></span>' . esc_html__( ' %1$s', 'write-blog' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
