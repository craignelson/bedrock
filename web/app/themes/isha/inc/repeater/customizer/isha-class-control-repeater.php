...
if( ! class_exists( 'Isha_Control_Repeater' ) ) {
    /**
     * Repeater control
     */
    class Isha_Control_Repeater extends WP_Customize_Control {
        ...
        public function __construct( $manager, $id, $args = array() ) {
            ...
            if ( empty( $this->button_label ) ) {
             ...
             $this->button_label = sprintf( esc_attr__( 'Add new %s', 'isha' ), $this->row_label['value'] );
            }
        ...
        public function enqueue() {    
            ...
            wp_enqueue_script( 'isha-customizer-repeater', get_template_directory_uri() . '/assets/customizer/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
            wp_enqueue_style( 'isha-customizer-repeater', get_template_directory_uri() . '/assets/customizer/repeater.css', null );
        }
        ...
        protected function render_content() {
            ...
            <?php if ( isset( $this->choices['limit'] ) ) : ?>
                ...
                printf( esc_attr__( 'Limit: %s rows', 'isha' ), esc_html( $this->choices['limit'] ) ); ?></p>
            <?php endif; ?>
        ...
        public function repeater_js_template() {
            ...
            <label>
                ...
                <span class="description customize-control-description">{{{ field.description }}}</span>
                <# } #>
                <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'isha' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
            ...
            <figure class="kirki-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'isha' ); ?>" >
                <# if ( field.default ) { #>
                <# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
                <img src="{{{ defaultImageURL }}}">
                <# } else { #>
                <?php esc_attr_e( 'No Image Selected', 'isha' ); ?>
                <# } #>
            </figure>
            
            <div class="actions">
                <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_attr_e( 'Remove', 'isha' ); ?></button>
                <button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'isha' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'isha' ); ?>" >
                    <# if ( field.default ) { #>
                    <?php esc_attr_e( 'Change Image', 'isha' ); ?>
                    <# } else { #>
                    <?php esc_attr_e( 'Add Image', 'isha' ); ?>
                    <# } #>
                </button>
                <# if ( field.default.id ) { #>
            ...
            <figure class="kirki-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'isha' ); ?>" >
                <# if ( field.default ) { #>
                ...
                <# } else { #>
                <?php esc_attr_e( 'No File Selected', 'isha' ); ?>
                <# } #>
            </figure>
            
            <div class="actions">
                <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
                <button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'isha' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'isha' ); ?>" >
                    <# if ( field.default ) { #>
                    <?php esc_attr_e( 'Change File', 'isha' ); ?>
                    <# } else { #>
                    <?php esc_attr_e( 'Add File', 'isha' ); ?>
                    <# } #>
                </button>
                <# if ( field.default.id ) { #>
                <input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
        ...
        <# }); #>
        <button type="button" class="button-link repeater-row-remove"><?php esc_attr_e( 'Remove', 'isha' ); ?></button>
    </div>
</li>
</script>
<?php
